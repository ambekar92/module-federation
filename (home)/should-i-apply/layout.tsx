...
const Layout = ({children}: PropsWithChildren) => {
  ...
  return (
    ...
    <Grid row className='height-full border-y border-y-05 border-base-lighter'>
    <NavBar />
    <div className='padding-x-2 flex-1 margin-bottom-2'>
      {children}
    </div>
    </Grid>
      <ButtonGroup className='flex flex-row flex-justify margin-y-2'>
        {!pathname.includes('ownership') && <Button type='button' onClick={handlePrevious} outline>Previous</Button>}
        <Button type='button' onClick={onSubmit}>{pathname.includes('results') ? 'Close' : 'Next'}</Button>
      </ButtonGroup>
    </FormProvider>
    </>
  )
