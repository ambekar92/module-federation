...
locals {
  container_environment = {
    ...
    NEXTAUTH_URL                         = "https://ucp-training.${local.env.domain_name}"
    NEXT_PRIVATE_LOCAL_WEBPACK           = "true" 
    NEXT_PUBLIC_API_URL                  = "https://ucp-api-training.${local.env.domain_name}/api/v1"
    ...
    NEXT_PUBLIC_WS_LIVE_NOTIFICATIONS    = "ws://ucp-training-communication.${local.env.domain_name}/communication/v1/live-notifications/1"
    REDIS_PORT                           = "6379"
...
module "ucp-wfe-training" {
  ...
  container_definitions = [
    {
      name         = "wfeService-training"
      image        = "222484291001.dkr.ecr.${local.region}.amazonaws.com/ucp-wfe:training"
      cpu          = 1024
      memory       = 3072
