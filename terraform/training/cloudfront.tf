# Alias Records
resource "aws_route53_record" "ipv4-training" {
  name            = "ucp-training.${local.env.domain_name}"
  type            = "A"
  ...
  alias {
    ...
    name                   = aws_cloudfront_distribution.distribution_training.domain_name
    zone_id                = aws_cloudfront_distribution.distribution_training.hosted_zone_id
  }
}
resource "aws_route53_record" "ipv6-training" {
  name            = "ucp-training.${local.env.domain_name}"
  type            = "AAAA"
  zone_id         = data.aws_route53_zone.selected.id
  allow_overwrite = true
...
# Distribution
resource "aws_cloudfront_distribution" "distribution_training" {
  aliases             = ["ucp-training.${local.env.domain_name}"]
  comment             = "ucp-wfe-training-${terraform.workspace}"
  enabled             = true
  http_version        = "http2"
  ...
  origin {
    ...
    domain_name         = "ucp-training-wfe.${local.env.domain_name}"
    origin_id           = "wfe"
