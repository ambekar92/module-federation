...
const middlewares: { [key: string]: any } = {
  ...
  '/dashboard/:path*': [
    async (request: NextRequest) => {
      ...
      if (
        isRole(permissions, Role.PRIMARY_QUALIFYING_OWNER) ||
        isRole(permissions, Role.CONTRIBUTOR)
      ) {
        return NextResponse.redirect(
          `${request.nextUrl.origin}/admin/dashboard`,
        )
      } else if (
        !isRole(permissions, Role.PRIMARY_QUALIFYING_OWNER) &&
        !isRole(permissions, Role.CONTRIBUTOR) &&
        !isRole(permissions, Role.ADMIN)
      ) {
        return NextResponse.redirect(
          `${request.nextUrl.origin}/user/dashboard/tasks`,
        )
      } else {
        return NextResponse.next()
      }
...
async function getData(request: NextRequest) {
  ...
  const email_password_auth_token = cookies.has('email_password_auth_token')
    ? JSON.parse(decrypt(cookies.get('email_password_auth_token')?.value) ?? '')
    : (null as unknown as LoginResponseUser)
  const maxgov_auth_token = cookies.has('maxgov_auth_token')
    ? JSON.parse(decrypt(cookies.get('maxgov_auth_token')?.value) ?? '')
    : (null as unknown as LoginResponseUser)
