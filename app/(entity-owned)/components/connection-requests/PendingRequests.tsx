...
const PendingRequests = () => {
  ...
  const scrollLeft = () => {
    if (scrollRef && scrollRef.current) {scrollRef.current.scrollBy({ left: -300, behavior: 'smooth' });}
  };
  ...
  useEffect(() => {
    setPendingData(pendingRequestsData as any);
    setReviewedData(reviewedRequestsData as any);
  }, [])

  const handleAcceptRequest = (param: any, id: any) => {
    const updatedItems = pendingData.filter((item: any) => item.id !== id);
    const selectedItem = pendingData.filter((item: any) => item.id === id) as any;
  ...
  return (
    <>
          ...
          <div className={styles['scroll-container']}>
            ...
            >
              {pendingData.length > 0 && pendingData.map((request, index) => (
                <Grid className={styles['usa-card']} key={index}>
      ...
      <ReviewedRequests
        reviewedRequestsData={reviewedData as any}
      />
