...
function HubMock() {
  ...
  const role = session?.data?.permissions?.[session?.data?.permissions?.length - 1]?.slug;
  ...
  return (
    <div style={{lineHeight: 1.5}}>
      ...
      <p>
        ...
        <TooltipIcon text='The HUBZone Eligibility Calculator is used to enter information about a business’ employees and where they work. Once the steps are completed, you’ll be able to see if your business appears to meet the HUBZone Program’s requirement that the principal office be located in a HUBZone and at least 35% of the business’ employees reside in a HUBZone.' />
      </p>
      ...
      <div className='display-flex flex-justify-end margin-top-5'>
        <a target="_blank" rel="noopener noreferrer" className='usa-button' href={`/hubzone?application_contributor_id=${contributorId}&user_id=${userId}&application_id=${applicationId}&role=`}>
					Start
        </a>
      </div>
    </div>
