...
import { selectApplication, setIsAddingOperator, setStep } from '../../redux/applicationSlice';
...
function ControlAndOperations({ children }: ControlAndOperationsProps) {
  ...
  const { operators, isAddingOperator } = useApplicationSelector(selectApplication);
  ...
  return (
    <>
      ...
      <div className="flex-fill" style={{ display: 'flex', gap: '1rem', flexDirection: 'column' }}>
        <GridContainer containerSize="widescreen" className={`width-full padding-y-2 margin-top-2 ${operators.length > 0 && 'bg-base-lightest'} ${isAddingOperator && 'bg-base-lightest'}`}>
          {children}
        </GridContainer>
