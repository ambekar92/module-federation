...
const DocumentCategoriesTable = async ({ searchParams }: { searchParams: { sortColumn: 'name' | 'description' | 'documentClass', sortOrder: 'asc' | 'desc', q: string, page: string } }) => {
    ...
    return (res as unknown as IDocument[]).sort((a, b) => searchParams.sortColumn === 'name' && searchParams.sortOrder === 'asc' ? a.name.localeCompare(b.name) :
      searchParams.sortColumn === 'name' && searchParams.sortOrder === 'desc' ? b.name.localeCompare(a.name) :
        searchParams.sortColumn === 'description' && searchParams.sortOrder === 'asc' ? a.description.localeCompare(b.description) :
          searchParams.sortColumn === 'description' && searchParams.sortOrder === 'desc' ? b.description.localeCompare(a.description) : 
        searchParams.sortColumn === 'documentClass' && searchParams.sortOrder === 'asc' ? a.documentClass.localeCompare(b.documentClass) :
          searchParams.sortColumn === 'documentClass' && searchParams.sortOrder === 'desc' ? b.documentClass.localeCompare(a.documentClass) : 
          0)
          .filter(item => `${item.name.toLowerCase()} ${item.description.toLowerCase()} ${item.documentClass.toLowerCase()}`.includes((searchParams.q ?? '').toLowerCase()))
