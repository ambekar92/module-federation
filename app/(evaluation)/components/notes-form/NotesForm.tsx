...
import { NOTES_PAGE, buildRoute } from '@/app/constants/url';
...
const NotesForm = ({ modalRef, createText='Create', showCancelBtn=true, noteEdited = null, richTextEditorSize }:
  ...
  return (
    <form onSubmit={handleSubmit(onNoteSave as SubmitHandler<FieldValues>)}>
      <p>
                After you create a note, you can view it in the
        {' '}<a href={buildRoute(NOTES_PAGE, {applicationId: Number(params.application_id)})}>Notes</a>{' '}
                section.
      </p>
      ...
      <ButtonGroup style={{ display: 'flex', marginTop: '1rem' }}>
        <Button type='submit'
        ...
        {showCancelBtn && <Button type='button' className="usa-button--outline"
          onClick={onCancel}>
                    Cancel
        </Button>}
      </ButtonGroup>
