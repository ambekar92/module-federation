...
const ReturnToFirmDataTable: React.FC = () => {
  ...
  const handleEdit = (id: any) => {
    setEditModal(true);
    const updatedItems = tableData.filter((item: any) => item.id === id);
              ...
              {tableData.map((item, index) => {
                return (
                  <tr key={index}>
                    ...
                    <td>{item.status}</td>
                    <td>
                      <span className={`${styles['actionButton']}`} onClick={() => handleEdit(item.id)}>{item.action1}</span>
                      <span className={`${styles['actionButton']} margin-left-4`} onClick={() => handleDelete(item.id)}>{item.action2}</span>
                    </td>
                  </tr>
                )
              })}
                ...
                <tbody>
                  <tr>
                    ...
                    <td>
                      ...
                      <span className={`${styles['actionButton']} margin-left-4`}>Close??</span>
                    </td>
                  </tr>
                </tbody>
              </Table>
