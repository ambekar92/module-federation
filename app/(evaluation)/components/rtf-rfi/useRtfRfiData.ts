...
export const useRtfRfiData = (application_id: string, permission: string) => {
  const { data: draftData, error } = useFetchOnce<IRTFItems[]>(`${permission === 'screener' ? DRAFT_RTF_ITEMS_ROUTE : RFI_DRAFT_ROUTE}/${application_id}`, fetcher);
