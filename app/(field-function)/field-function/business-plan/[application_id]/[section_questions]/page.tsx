...
const BusinessPlanPage = () => {
  ...
  function onContinue() {
    const current = navItems?.error ? " " : navItems?.find(q => q.title === title);
    if (!current || !navItems) { setShowNextButton(false); return; }
    const currIdx = navItems?.error ? 0 : navItems?.indexOf(current);
  ...
  useEffect(() => {
    if (navItems && data) {      
      const currentSection = navItems?.error ? " " : navItems?.find(item => item.url.includes(params.section_questions));
      if (currentSection) {
        setTitle(currentSection.title);
      }
      const current = navItems?.error ? " " : navItems?.find(item => item.url.includes(params.section_questions));
      const currIdx = navItems?.error ? 0 : (current ? navItems?.indexOf(current) : 0);
