...
export const ClaimBusinessSchema = z.object({
  ...
  }).regex(ueiValidation, 'UEI must contain only alphanumeric characters and be 12 characters long.'),

  cageCode: z.string()
    .min(0) // Allows empty strings
    .regex(cageCodeValidation, 'CAGE Code must contain only alphanumeric characters and be 5 characters long.')
  ...
  }).regex(tinValidation, 'TIN must contain only numeric digits and be 9 digits in length.'),
