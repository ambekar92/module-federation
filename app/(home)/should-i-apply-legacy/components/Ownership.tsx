...
const Ownership = () => {
  ...
  useEffect(() => {
    if (displayPrograms) {
      const currentBusinessLocation = getValues('businessLocation');
      const currentSocialDisadvantages = getValues('socialDisadvantages');

      if(checkIfUSAddress(currentBusinessLocation)) {
        const programs = programOptions.filter(program =>
          program.disadvantages.some(disadvantage =>
            currentSocialDisadvantages.includes(disadvantage)
          )
        );
  ...
  }, [displayPrograms, getValues]);

  // Handle changes in the Select component
  const handleSelectChange = (newValue: MultiValue<SelectOption>) => {
    setSelectedOptions([...newValue]);
    const values = newValue.map((option) => option.value);
    const currentValues = getValues('socialDisadvantages');

    // Filter out any 'veteran' or 'female' values which are managed separately
    const filteredCurrentValues = currentValues.filter(value => value === 'veteran' || value === 'female');

    setValue('socialDisadvantages', [...filteredCurrentValues, ...values], { shouldValidate: true });
  };

  const handleSubmit = async () => {
    // Trigger validation for all form fields except the select.
    const isValid = await trigger();

    // Check if any options are selected in the Select component.
    if (selectedOptions.length === 0) {
      setDisplayDisadvantageError(true);
      return; // Stop the function if no options are selected.
    } else {
      setDisplayDisadvantageError(false); // Clear any error.
    }

    if (isValid) {
      const userData = getValues();

      // Start with the current selections from the Select component.
      let updatedDisadvantages = selectedOptions.map(option => option.value);

      // Conditionally add 'female' if gender is 'F' and it's not already included.
      if (userData.gender === 'F' && !updatedDisadvantages.includes('female')) {
        updatedDisadvantages.push('female');
      } else if (userData.gender !== 'F') {
        updatedDisadvantages = updatedDisadvantages.filter(disadvantage => disadvantage !== 'female');
      }

      // Conditionally add 'veteran' if veteran is 'Yes' and it's not already included.
      if (userData.veteran === 'Yes' && !updatedDisadvantages.includes('veteran')) {
        updatedDisadvantages.push('veteran');
      } else if (userData.veteran !== 'Yes') {
        updatedDisadvantages = updatedDisadvantages.filter(disadvantage => disadvantage !== 'veteran');
      }

      // Conditionally add 'disabledVeteran' if veteran is 'Yes' and it's not already included.
      if ((userData.disabledVeteran === 'Yes' && userData.veteran === 'Yes') && !updatedDisadvantages.includes('disabledVeteran')) {
        updatedDisadvantages.push('disabledVeteran');
      } else if (userData.disabledVeteran !== 'Yes') {
        updatedDisadvantages = updatedDisadvantages.filter(disadvantage => disadvantage !== 'disabledVeteran');
      }

      // Update the socialDisadvantages in the form state.
      setValue('socialDisadvantages', updatedDisadvantages, { shouldValidate: true });

      if (userData.usCitizen === 'No') {
        setIsCitizen(false);
        setFilteredEligiblePrograms([]); // Set to empty because user is not eligible.
      } else {
        setIsCitizen(true);
        updateProgramEligibility(userData, updatedDisadvantages);
      }
  ...
  const updateProgramEligibility = (userData: Inputs, socialDisadvantages: string[]) => {
    if (checkIfUSAddress(userData.businessLocation)) {
      const eligiblePrograms = programOptions.filter(program =>
        program.disadvantages.some(disadvantage =>
          socialDisadvantages.includes(disadvantage)
        )
      );
      setFilteredEligiblePrograms(eligiblePrograms);
    } else {
      setFilteredEligiblePrograms([]); // Clear the list if the address doesn't qualify.
    }
  };
    ...
    <GridContainer containerSize='widescreen'>
        ...
        isCitizen === false
          ...
          : filteredEligiblePrograms.length > 0
            ? <section className={`${Styles.section} ${Styles.programs}`}>
              ...
              <Grid row gap col={12}>
                <Grid tablet={{ col: 6 }} className={`width-full ${Styles.program_grid}`}>
                  {filteredEligiblePrograms.map((program, index) => (
                    <ProgramCard key={index} program={program.name} description={program.description} details={program.details} />
                  ))}
                </Grid>
              </Grid>
            </section>
      ...
      <section>
        ...
        <form className='ownership-form'>
          ...
          {isVeteran === 'Yes' && (
            <div className='display-flex flex-column flex-align-end margin-top-4'>
              <div>
                ...
                <>
                  <div className={`display-flex flex-wrap ${Styles.column_gap}`}>
                    ...
                    <div className='display-flex flex-auto flex-align-center'>
                      ...
                      <a className='margin-left-1' target="_blank" rel="noopener noreferrer" href="/application/hubzone">Hubzone Calculator</a>
                    </div>
                  </div>
                  {error && (
                    <div className={Styles.error_text}>
