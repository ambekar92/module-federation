...
import { deleteEmployee, selectForm, setEditingEmployee } from '../store/formSlice';
...
const EmployeeTable = ({ reset, setValue }: Props) => {
  ...
  const handleDeleteEmployee = (index: number) => {
    if (editingEmployee !== null && employees[index].id === editingEmployee.id) {
      reset({
        name: '',
        role: undefined,
        position: '',
  ...
  const handleEditEmployee = (index: number) => {
    ...
    if (employee) {
      ...
      setValue('licenseHolder', employee.licenseHolder);
      ...
      <table className="usa-table usa-table--borderless">
        ...
        <tbody>
          <tr>
            ...
            <td className="bold">License Holder</td>
          ...
          {employees.map((employee, index) => (
            <tr key={index}>
              ...
              <td>{employee.licenseHolder}</td>
              <td>
                <button type="button" className="icons" onClick={() => handleEditEmployee(index)}>
                  <svg
                    width="20"
                    height="19"
                    viewBox="0 0 20 19"
                ...
                <button type="button" className="icons" onClick={() => handleDeleteEmployee(index)}>
                  <svg
                    width="18"
                    height="19"
                    viewBox="0 0 18 19"
